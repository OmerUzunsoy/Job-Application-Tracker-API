...
{
    Task<IReadOnlyList<JobApplicationDto>> GetAllAsync(Guid userId, GetJobApplicationsQuery query, CancellationToken cancellationToken = default);
    ...
    Task DeleteAsync(Guid userId, Guid jobApplicationId, CancellationToken cancellationToken = default);
    ...
    Task<DashboardSummaryDto> GetDashboardAsync(Guid userId, CancellationToken cancellationToken = default);
}
